...
def run_madm(landmarks, target_mudra='', min_frames=None, label='Right'):
    ...
    try:
        ...
        if not skip_rule_veto:
            ...
            # 3. Pataka (Flat Palm - All fingers up and straight, thumb tucked)
                  ...
                  raw_angles.get("thumb", 0) > 140 and
                  dist_lm(lm_wrapper, 4, 5, palm_size) <= 0.40):
                raw_name = "pataka"
                raw_conf = 95.0
                rule_triggered = True
                print(f"[RULE VETO] Override to Pataka (Flat Palm)")

            # 3.b Ardhachandra (Flat Palm - All fingers up and straight, thumb wide)
            elif (raw_angles.get("index", 0) > 160 and 
                  raw_angles.get("middle", 0) > 160 and 
                  raw_angles.get("ring", 0) > 160 and 
                  raw_angles.get("pinky", 0) > 160 and
                  raw_angles.get("thumb", 0) > 140 and
                  dist_lm(lm_wrapper, 4, 5, palm_size) > 0.40):
                raw_name = "ardhachandra"
                raw_conf = 95.0
                rule_triggered = True
                print(f"[RULE VETO] Override to Ardhachandra (Wide Thumb Palm)")
...
def detect_frame():
    global ema_landmarks, ema_probs, frame_counter, current_mudra, no_hand_counter
...
def detect_landmarks():
    global current_mudra, is_double_mode, no_hand_counter, ema_landmarks, ema_probs
    try:
...
# Add this route to flask_app.py just before `# =============================================================================
...
def predict_double():
    ...
    try:
        ...
        left_lms = body.get('left_landmarks', None)
        right_lms = body.get('right_landmarks', None)

        if left_lms or right_lms:
            # Direct landmarks path
            def to_pts(raw):
                if not raw: return None
                return [LM(float(p['x']), float(p['y']), float(p['z'])) for p in raw]

            left_lm = to_pts(left_lms)
            right_lm = to_pts(right_lms)
...
ADAVU_MODEL_PATH = os.path.join(MODEL_DIR, "adavu_model.pkl")
_adavu_model = None
_adavu_model_classes = []

def _load_adavu_model():
    global _adavu_model, _adavu_model_classes
    if _adavu_model is not None:
        return True
    if not os.path.exists(ADAVU_MODEL_PATH):
        return False
    try:
        with open(ADAVU_MODEL_PATH, 'rb') as f:
            _adavu_model = pickle.load(f)
        _adavu_model_classes = list(_adavu_model.classes_)
        print(f"[Adavu model] Loaded. Classes: {_adavu_model_classes}")
        return True
    except Exception as e:
        print(f"[Adavu model] Load error: {e}")
        return False
...
def predict_pose():
    try:
        ...
        if not landmarks or len(landmarks) < 33:
            return jsonify({"detected": False, "name": "No Body Present", "confidence": 0})

        if not _load_adavu_model():
            return jsonify({
                "detected": True,
                "name": "Pose Tracked",
                "confidence": 85.0,
                "message": "Rule-based stance active. Model file adavu_model.pkl optional."
            })
        ...
        torso_height = np.sqrt((sc_x - hc_x)**2 + (sc_y - hc_y)**2 + (sc_z - hc_z)**2) or 1.0

        features = []
        for lm in landmarks[:33]:
            features.extend([
                (lm['x'] - hc_x) / torso_height,
                (lm['y'] - hc_y) / torso_height,
                (lm['z'] - hc_z) / torso_height
            ])

        probs = _adavu_model.predict_proba([features])[0]
        max_idx = np.argmax(probs)
        conf = float(probs[max_idx]) * 100.0
        predicted_name = str(_adavu_model_classes[max_idx])

        return jsonify({
            "detected": conf >= 45.0,
            "name": predicted_name if conf >= 45.0 else "Unknown Stance",
            "confidence": round(conf, 1),
            "is_stable": conf >= 70.0
        })
    except Exception as e:
        print(f"[predict_pose] Error: {e}")
...
def evaluate_sequence_image():
    ...
    try:
        ...
        return jsonify({
            ...
            "reference_angles": best_ref_angles,
            "feedback": feedback if feedback else ["Excellent posture alignment!"]
        })
    except Exception as e:
